...
books_data = [
    {
        ...
        "copies_owned": 0,
        "image": "assets/book/book_cover.webp",
        "language": "English",
        "pages": 464,
        "rating_count": 12500,
    },
...
def seed():
    ...
    with engine.begin() as conn:
        ...
        conn.exec_driver_sql("DROP TABLE IF EXISTS book_author CASCADE")
        conn.exec_driver_sql("DROP TABLE IF EXISTS books CASCADE")
        conn.exec_driver_sql("DROP TABLE IF EXISTS authors CASCADE")
    ...
    try:
        ...
        # Insert authors
        print(f"Seeding {len(authors_data)} authors...")
        author_name_to_id = {}
        for a in authors_data:
            aid = a.get("id")
            first = a.get("first_name")
            last = a.get("last_name")
            if aid and first is not None:
                db.add(Author(id=aid, first_name=first, last_name=last))
                full_name = f"{first} {last}".strip().lower()
                author_name_to_id[full_name] = aid
        ...
        for book_data in books_data:
            ...
            # Map author name to author_id if needed
            if "author_id" not in book_data or not book_data["author_id"]:
                if "author" in book_data and book_data["author"]:
                    author_name = str(book_data.pop("author")).strip().lower()
                    ...
                    if aid is None:
                        ...
                        aid = str(len(author_name_to_id) + 1)
                        author_name_to_id[author_name] = aid
                        db.add(Author(id=aid, first_name=first, last_name=last))
                    book_data["author_id"] = aid
